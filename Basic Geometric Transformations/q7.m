...
for i = 1:loops
    ...
    [ball_temp] = imwarp(ball_sc,ball_ref,tform,'cubic','FillValues',1);
    [mask_temp] = imwarp(mask_sc,mask_ref,tform,'cubic','FillValues',1);
    %% Place rotated image appropriately
    [m,n,~] = size(ball_temp);
    t_y(i) = 400 - floor(n/2) + 2*i;
    t_x(i) = max(x) - x(i) + 1;
