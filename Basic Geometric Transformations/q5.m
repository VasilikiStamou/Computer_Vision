...
[mill] = imwarp(mill,tform_tr,'Interp','linear','FillValues',1);
[mill_mask,imref_temp] = imwarp(mill_mask,tform_tr,...
    'Interp','linear','FillValues',1);
%% Main Loop
for i = 1:frames
    ...
    A = [ cosd(theta) sind(theta) 0;
         -sind(theta) cosd(theta) 0;
              0           0       1];
   tform_rt = affine2d(A');
   [im_] = imwarp(mill,imref_temp,tform_rt,'Interp','linear','FillValues',1);
